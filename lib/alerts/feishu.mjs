...
import { translateContent } from '../llm/translate.mjs';
...
export class FeishuAlerter {
  ...
  _formatAlert(evaluation, delta, source = 'llm') {
    return [
      `【CRUCIX ${this._tierLabel(evaluation.tier)}】${evaluation.headline || '信号告警'}`,
      evaluation.reason ? `原因：${evaluation.reason}` : '',
      `方向：${delta?.summary?.direction || 'mixed'} | 变化数：${delta?.summary?.totalChanges || 0} | 关键变化：${delta?.summary?.criticalChanges || 0}`,
      `置信度：${this._confidenceLabel(evaluation.confidence)} | 来源：${this._sourceLabel(source)}`,
      evaluation.actionable ? `建议：${evaluation.actionable}` : '',
      evaluation.signals?.length ? `信号：${evaluation.signals.join(' · ')}` : '',
  ...
  async evaluateAndAlert(llmProvider, delta, memory) {
    ...
    const alertText = this._formatAlert(evaluation, delta, evalSource);
    const finalText = llmProvider?.isConfigured
      ? await translateContent(llmProvider, alertText, 'zh-CN')
      : alertText;
    const sent = await this.sendMessage(finalText);
